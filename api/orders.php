...
if ($method === 'POST') {
    ...
    $paymentMethod = in_array($input['payment_method'] ?? '', ['cash']) ? $input['payment_method'] : 'cash';
    $deliveryAddress = '';
    ...
    try {
        $stmt = $db->prepare("
            INSERT INTO orders (order_number, customer_name, customer_phone, delivery_type, delivery_address,
                promo_code, discount_amount, subtotal, delivery_fee, total, payment_method, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'new')
        ");
        $stmt->execute([
            $orderNumber,
